...
걸음은 멈춰도, 기록은 멈추지 않도록. 기록을 잃지 않는 이동 기록 앱.

## 포지셔닝

재미는 KYRO, 신뢰는 balmi. **Balmi × VASA** — 발걸음에서 혈관까지.

사용자에게 보이는 문구에 타사 클라우드 브랜드명, 동반, 장례 표현을 넣지 않는다.
가혹한 크래시 표현(`앱이 죽어도`) 대신 오프라인·복구 톤을 쓴다.

브랜드·컬러 사용 행렬: [`docs/BRAND.md`](BRAND.md).
...
브랜드 UI: 부드럽고 친근한 한국어. 의료/장례 톤 아님. 이름 **balmi**.
RECORD 슬로건 `걸음은 멈춰도, 기록은 멈추지 않도록.` · HEALTH `발걸음에서 혈관까지.`
설정/소개에 **× VASA** 기술 크레딧(의료 점수가 아님). 컬러: **고구마(Sweet Potato)=브랜드/경로**, **Active Orange=움직임/현재 위치** — 상세는 [`BRAND.md`](BRAND.md).
